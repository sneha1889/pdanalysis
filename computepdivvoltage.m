function pdivVoltage = computepdivvoltage(voltageValues,actTimeStamp, timeStamp,pdivRow)
%% function maximumValue = pdivvoltage(voltageValues,actTimeStamp, timeStamp,thresholdRowValue)
actTimestamprow = 0;
pdivVoltage=0;
n=0;
zeroCrossingTime=0;
...
zeroCrossingTime = timeStamp(pdivRow);
...
for i = actTimestamprow:size(voltageValues,1)
    ...
    pdivVoltage = max(abs(analysisArray));
end